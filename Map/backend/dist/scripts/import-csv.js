...
async function importCsv() {
  ...
  `);
    ...
    const brandInitial = (row) => (row.brand_initial ?? row.brand_name ?? "").trim().toUpperCase();
    const insertMany = (rows) => {
        ...
        try {
            for (const row of rows) {
                ...
                if (!initial)
                    continue;
                insert.run(row.id, initial, latitude, longitude, row.state, row.city, row.zipcode ?? null, row.status, row.type ?? null, row.channel ?? null);
